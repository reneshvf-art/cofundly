...
function initA11yBar() {
  ...
  if (!bar) return;
  if (sessionStorage.getItem('cf_a11y_dismissed')) {
  ...
  bar.querySelector('[data-a11y="close"]')?.addEventListener('click', () => {
    bar.style.display = 'none';
    sessionStorage.setItem('cf_a11y_dismissed', '1');
  });
}
...
function initAlertBar() {
  ...
  if (!bar) return;
  if (sessionStorage.getItem('cf_alert_dismissed')) {
    bar.style.display = 'none';
    return;
  }
  bar.querySelector('.help-bar-close')?.addEventListener('click', () => {
    bar.style.display = 'none';
    sessionStorage.setItem('cf_alert_dismissed', '1');
  });
}
